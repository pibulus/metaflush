...
import { getThemeColor } from './mascotTheme';
...
function initGradientPositionAnimation(themeId, gradient) {
	...
	const speedFromCss = parseFloat(
		getCssVariable(`mascot-${themeId}-position-speed`, config.position.speed)
	);
	const amplitudeFromCss = parseFloat(
		getCssVariable(`mascot-${themeId}-position-amplitude`, config.position.amplitude)
	);
...
function initStopColorAnimations(themeId, stops) {
	...
	stops.forEach((stop, index) => {
		...
		const baseOpacity = parseFloat(
			getCssVariable(
				`mascot-colorpulse-${opacityRangeKey}-opacity-min`,
				timingConfig.opacityRanges[opacityRangeKey][0]
			)
		);
		const brightOpacity = parseFloat(
			getCssVariable(
				`mascot-colorpulse-${opacityRangeKey}-opacity-max`,
		...
		const duration =
			parseFloat(getCssVariable('mascot-colorpulse-duration', timingConfig.duration * 1000)) / 1000;
		const staggerDelay = parseFloat(
			getCssVariable('mascot-colorpulse-stagger', timingConfig.staggerDelay)
		);
