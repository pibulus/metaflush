/**
 * Mascot Animation State Store
 *
 * A reactive store for managing mascot animation states and transitions
 ...
 *
 * ⚠️ SINGLE-INSTANCE: this store is a module-level singleton, and the
 * gradient/animation/blink services hold module-level state too. Render only
 * ONE <Mascot> per page (the real usage — a mascot is a hero element). Two
 * instances would share/clobber this state. To support multiple, convert the
 * `createMascotStateStore()` factory below into a per-component instance passed
 * via Svelte context.
 */
...
function createMascotState(overrides = {}) {
	return {
...
/**
 * Create the mascot animation state machine
 */
function createMascotStateStore() {
	// Internal state store (make sure this 'state' is used throughout)
	const _state = writable(createMascotState({ current: ANIMATION_STATES.INITIAL }));
	...
	function debugLog(message, level = 'log') {
		...
		console[level](`[MascotState] ${message}`);
	}
	...
	/**
	 * Apply wobble animation to the mascot
	...
	function applyWobbleAnimation(type) {
		...
		if (!spinPivot) {
			debugLog(`[Wobble] Could not find .mascot-spin-pivot target for ${type}`, 'warn');
			return;
		}
		...
		const wobbleGroup = spinPivot.querySelector('.mascot-wobble-group');
	...
	function checkAndSetFirstVisit() {
		if (browser) {
			...
			const hasSeenMascot = sessionStorage.getItem('mascot-seen');
			const firstVisit = !hasSeenMascot;

			if (firstVisit) {
				debugLog('First visit detected, updating store.');
				sessionStorage.setItem('mascot-seen', 'true');
	...
	function completeFirstVisit() {
		if (browser) {
			document.body.setAttribute('data-mascot-animated', 'true');
			_state.update((s) => ({ ...s, isFirstVisit: false }));
		}
	}
	...
	/**
	 * Wakes the mascot up from ASLEEP state.
	 */
	function wakeUp() {
		const currentStoreState = get(_state);
	...
	return {
		..._mascotState, // Spread existing methods from internal object
		// Add derived stores
...
export const mascotState = createMascotStateStore();
