/**
 * Svelte Action for Mascot Initial Animation Sequence
 *
 * This action orchestrates the initial animation for the mascot:
...
import { ANIMATION_TIMING } from '../animationConfig.js';

export function initialMascotAnimation(node, initialParams) {
	...
	function runSetup(params) {
		...
		blinkTimeoutId = setTimeout(() => {
			...
			if (
				...
				if (currentDebugState) {
					console.warn(
						'[Action initialMascotAnimation] Could not perform double blink during timeout. Missing blinkService, eye elements, or function.'
					);
				}
