/**
 * Mascot Animation Configuration
 * Centralized configuration for mascot animation parameters
 */
...
export const WOBBLE_CONFIG = {
	...
	DURATION: 390, // Duration of mascot wobble animation (ms) - Increased for more noticeable effect
...
export const EYE_CONFIG = {
	...
	X_MULTIPLIER: 6, // Horizontal movement multiplier for eyes (increased for better visibility)
	Y_MULTIPLIER: 3, // Vertical movement multiplier for eyes (increased for better visibility)
	...
	SMOOTHING: 0.45, // Smoothing factor for eye movement (0-1) - Higher = more responsive
...
export const ANIMATION_BEHAVIORS = {
	...
	[ANIMATION_STATES.EASTER_EGG]: {
		...
		// Note: The actual animation (spin) is applied via CSS class in Mascot.svelte
	},
...
export function generateAnimationCssVariables() {
	...
	cssVars += `--mascot-transition: ${ANIMATION_TIMING.BASE_TRANSITION};\n`;
	cssVars += `--mascot-shimmer-duration: ${ANIMATION_TIMING.SHIMMER_DURATION};\n`;
	cssVars += `--mascot-shimmer-ease: ${ANIMATION_TIMING.SHIMMER_EASE};\n`;
	cssVars += `--mascot-gradientshift-duration: ${ANIMATION_TIMING.GRADIENT_SHIFT_DURATION};\n`;
	cssVars += `--mascot-colorpulse-duration: ${ANIMATION_TIMING.COLOR_PULSE_DURATION};\n`;
	cssVars += `--mascot-colorpulse-stagger: ${ANIMATION_TIMING.COLOR_PULSE_STAGGER};\n`;
	cssVars += `--mascot-eye-blink-transition: ${EYE_CONFIG.BLINK_TRANSITION};\n`;

	// Flow animation durations
	cssVars += `--mascot-peach-flow-duration: ${ANIMATION_TIMING.PEACH_FLOW_DURATION};\n`;
	cssVars += `--mascot-mint-flow-duration: ${ANIMATION_TIMING.MINT_FLOW_DURATION};\n`;
	cssVars += `--mascot-bubblegum-flow-duration: ${ANIMATION_TIMING.BUBBLEGUM_FLOW_DURATION};\n`;
	cssVars += `--mascot-rainbow-flow-duration: ${ANIMATION_TIMING.RAINBOW_FLOW_DURATION};\n`;

	// Flow animation easing
	cssVars += `--mascot-flow-ease: ${ANIMATION_TIMING.FLOW_EASE};\n`;

	// Add global animation parameters
	cssVars += '\n/* Global Animation Parameters */\n';
	cssVars += `--mascot-shimmer-opacity-min: ${ANIMATION_EFFECTS.SHIMMER_OPACITY_MIN};\n`;
	cssVars += `--mascot-shimmer-opacity-max: ${ANIMATION_EFFECTS.SHIMMER_OPACITY_MAX};\n`;
	cssVars += `--mascot-gradient-shift-magnitude: ${ANIMATION_EFFECTS.GRADIENT_SHIFT_MAGNITUDE};\n`;

	// Color pulse opacity ranges
	cssVars += `--mascot-colorpulse-start-opacity-min: ${ANIMATION_EFFECTS.COLOR_PULSE_START_OPACITY_MIN};\n`;
	cssVars += `--mascot-colorpulse-start-opacity-max: ${ANIMATION_EFFECTS.COLOR_PULSE_START_OPACITY_MAX};\n`;
	cssVars += `--mascot-colorpulse-mid-opacity-min: ${ANIMATION_EFFECTS.COLOR_PULSE_MID_OPACITY_MIN};\n`;
	cssVars += `--mascot-colorpulse-mid-opacity-max: ${ANIMATION_EFFECTS.COLOR_PULSE_MID_OPACITY_MAX};\n`;
	cssVars += `--mascot-colorpulse-end-opacity-min: ${ANIMATION_EFFECTS.COLOR_PULSE_END_OPACITY_MIN};\n`;
	cssVars += `--mascot-colorpulse-end-opacity-max: ${ANIMATION_EFFECTS.COLOR_PULSE_END_OPACITY_MAX};\n`;

	// Theme-specific parameters - Peach
	cssVars += '\n/* Peach Theme Animation Parameters */\n';
	Object.entries(ANIMATION_EFFECTS.PEACH).forEach(([key, value]) => {
		cssVars += `--mascot-peach-${key.toLowerCase()}: ${value};\n`;
	});

	// Theme-specific parameters - Mint
	cssVars += '\n/* Mint Theme Animation Parameters */\n';
	Object.entries(ANIMATION_EFFECTS.MINT).forEach(([key, value]) => {
		cssVars += `--mascot-mint-${key.toLowerCase()}: ${value};\n`;
	});
	...
	Object.entries(ANIMATION_EFFECTS.BUBBLEGUM).forEach(([key, value]) => {
		cssVars += `--mascot-bubblegum-${key.toLowerCase()}: ${value};\n`;
	});
	...
	Object.entries(ANIMATION_EFFECTS.RAINBOW).forEach(([key, value]) => {
		cssVars += `--mascot-rainbow-${key.toLowerCase()}: ${value};\n`;
	});
...
export function injectAnimationVariables() {
	...
	let styleElement = document.getElementById('mascot-animation-vars');
	if (!styleElement) {
		styleElement = document.createElement('style');
		styleElement.id = 'mascot-animation-vars';
...
// Rely on themeStore.js and Mascot.svelte::initDynamicStyles for variable injection
