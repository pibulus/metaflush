...
export function buildMascotPersonality({ seed = 0, date = new Date() } = {}) {
	...
	return {
		...
		style: [
			`--mascot-float-x: ${floatX}px`,
			`--mascot-float-y: ${floatY}px`,
			`--mascot-float-tilt: ${floatTilt}deg`,
			`--mascot-float-scale: ${floatScale}`,
			`--mascot-float-speed: ${floatSpeed}s`,
			`--mascot-float-delay: ${floatDelay}s`
		].join('; ')
	};
}
